...
class HistoryStore:
    ...
    def add_tag(self, idx: int, tag: str):
        if 0 <= idx < len(self.items):
            t = (self.items[idx].tags or "").strip()
            tags = [x for x in t.split(",") if x] if t else []
            if tag not in tags:
                tags.append(tag)
            self.items[idx].tags = ",".join(tags)
            self._save()

    def remove_item(self, item: HistoryItem):
        self.items = [it for it in self.items if not (
            it.expr == item.expr and it.base_from == item.base_from and
            it.base_to == item.base_to and it.result == item.result and it.timestamp == item.timestamp
        )]
        self._save()
    ...
    def list_items(self, query: str = "", sort_mode: str = "time_desc") -> List[HistoryItem]:
        ...
        if q:
            if q.startswith("#"):
                tag = q[1:]
                data = [i for i in data if tag in (i.tags or "")]
            elif q in {"fav","star"}:
    ...
    def export_csv(self, path: str, rows: Iterable[HistoryItem] = None):
        ...
        with open(path, "w", newline="", encoding="utf-8-sig") as f:
            w = csv.writer(f)
            w.writerow(["timestamp", "expr", "base_from", "base_to", "result", "favorite", "tags"])
            for it in rows:
                w.writerow([it.timestamp, it.expr, it.base_from, it.base_to, it.result, int(it.favorite), it.tags or ""])
        ...
        for row in rows[start:]:
            ...
            try:
                if has_header:
                    ...
                    it = HistoryItem(
                        timestamp=row[0], expr=row[1], base_from=int(row[2]), base_to=int(row[3]), result=row[4],
