...
class App(ctk.CTk):
    ...
    def _build_left_panel(self, parent: ctk.CTkFrame):
        ...
        self.ent_input = ctk.CTkEntry(
            inp, textvariable=self.var_input, placeholder_text="예: 1011.01  또는  A.F + 10"
        )
        ...
        # --- 요약 블록 (행 프레임 내부에서 위젯 생성: 부모/배치 일치) ---
    ...
    def _mk_summary_row(self, parent, r, label_text):
        ...
        entry = ctk.CTkEntry(fr, takefocus=False)  # 포커스 튐 방지
    ...
    def _style_treeview(self):
        ...
        if mode == "Dark":
            bg, fg = "#1f1f1f", "#eaeaea"
            sel_bg, sel_fg = "#3a7ebf", "#ffffff"
            hdr_bg, hdr_fg = "#262626", "#eaeaea"
        else:
            bg, fg = "#ffffff", "#222222"
            sel_bg, sel_fg = "#cde4ff", "#000000"
    ...
    def _copy_and_flash(self, text: str):
        ...
        # 결과칸 테두리만 잠깐 하이라이트 (fg_color 건드리지 않음)
        try:
    ...
    def on_convert(self):
        ...
        self._copy_and_flash(out)  # 성공 시 자동복사 + 하이라이트
        self.txt_steps.configure(state="normal")
        self.txt_steps.delete("1.0", "end")
        for s in steps:
